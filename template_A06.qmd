...
[https://your-github-repo-url](https://github.com/tylerbrown10/Brown_Tyler_201_A06)
...
for(player in names(player_info)){
  ppg[[player]] <- player_info[[player]]$points / player_info[[player]]$games_played
}
print(ppg)
```
...
ppg_rounded <- sapply(ppg, round, 2)
...
new_matrix <- apply(mat_rand, MARGIN = 1:2, round_matrix)
...
average_temp <- colMeans(max_temps[, -(1:2)])
...
col_medians <- apply(Colorado[, 3:14], 1, median)
print(paste0("The maximum median annua temperature of ", max(col_medians), " occurred in ", Colorado[[2]][[which.max(col_medians)]], " and the minimum median annual temperature of ", min(col_medians), " occurred in ", Colorado[[2]][[which.min(col_medians)]], "."))
